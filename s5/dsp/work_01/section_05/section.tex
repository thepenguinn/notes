...
Now we have \mintinline{python}{gen_dft_seq}, next we need a to compute $32$
...
\begin{minted}[breaklines, autogobble, mathescape] {python}
    ...
    for cplx in cplx_seqs:
        ...
        for samp_freq in samp_freqs:
            ...
            data.to_csv(
                "../data/dft_" + cplx["name"] + "_" + samp_freq + ".csv",
                sep = " ", index_label = "freq"
            )
\end{minted}

\subsection{DFT of Complex A}
...
\begin{figure} [H]
    ...
    \adjustbox{max width = 1\textwidth} {
        \includegraphics[height = 0.8\textheight] {tikzpics/plotDftComplexA.pdf}
    }
    \captionof{figure} {
