...
\newcounter{whereFootNote}
\newcounter{whatFootNote}
...
\begin{itemize} [label=]
    ...
    \item \textbf{Way Higher Sampling Frequency:} In Figure \ref{plt:higherSampFreq} we can almost
        see the two \emph{frequencies}. But if \emph{increase} \emph{sampling frequency} again
        without \emph{increasing} \emph{sample count}\footnote{in our case it was $32$ and $64$.},
        the \textsc{dtft} or \textsc{dft} might lose those \emph{frequency information}.
        It seems like the \emph{increase} the \emph{sample count} with \emph{sampling frequency}
        in order to get a proper \emph{frequency spectrum}.
